-- LNU Skills Commission Supabase schema
...
drop policy if exists "chat attachments lnu read" on storage.objects;
create policy "chat attachments lnu read"
on storage.objects
for select
using (bucket_id = 'chat-attachments' and public.is_lnu_email());

drop policy if exists "chat attachments owner create" on storage.objects;
create policy "chat attachments owner create"
on storage.objects
for insert
with check (
